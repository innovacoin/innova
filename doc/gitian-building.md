...
*Setup instructions for a Gitian build of Innova Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Innova
...
- [Building Innova Core](#building-innova-core)
...
Clone the git repositories for Innova Core and Gitian.
...
in the Innova Core repository under 'Fetch and build inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Innova Core
----------------

To build Innova Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Innova Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/innova/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/innovacoin/innova
...
For example:
```bash
URL=https://github.com/crowning-/innova.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit innova=${COMMIT} --url innova=${URL} ../innova/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit innova=${COMMIT} --url innova=${URL} ../innova/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit innova=${COMMIT} --url innova=${URL} ../innova/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the innova git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../innova/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/innova.git
SIGPATH=/some/root/path/innova-detached-sigs.git

./bin/gbuild --url innova=${BTCPATH},signature=${SIGPATH} ../innova/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/innova-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/innova-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/innova-osx-build.assert
```
...
[innova/gitian.sigs](https://github.com/innovacoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
